...
%%		\State $\textit{\alpha=0.1}$\\
%%		\State $\textit{p=1/Nombre_de_catégorie}$\\
		
%%		\State $\textit{dés l'authentification d'un utilisateur i charger ses préférences}$\\
%%		\State $\textit{preférences = préférences(i)}$\\
		
%%		\If {$\textit{Selection_Article}$}
%%		\State $\textit{\[p(catégorie_article) = (1-{\alpha}) {p} + {\alpha}\]}$\\
%%		\emph{Pour toute catégorie!=catégorie_article}:
%%		\State $\textit{\[p(catégorie) = (1-{\alpha}) {p} $\\
...
%%		\State $\textit{dés l'authentification d'un utilisateur i charger ses préférences}$\\
%%		\State $\textit{preférences = préférences(i)}$\\
%%		\emph{Pour chaque utilisateur j dans la base des profils}:
		%%\If {$\textit{Distance_euclidienne}(i,j)>= \textit{Seuil}(j)$}
%%		\State $\textit{préférences += préférences(j)}$\\
%%		\State \textbf{Fin Si};	
%%		\State \textbf{Fin Pour};
%%		\EndProcedure
%%	\end{algorithmic}
